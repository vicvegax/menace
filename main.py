...
class Player:
  def __init__(self, HC):
    #self.XO =  'x' if XO.lower() == 'x' else 'o'
    self.HC = 'h' if HC.lower() == 'h' else 'c'
    self.row = None
    self.col = None
  ...
  def c_play(self):
    global board
    while not (self.row and self.col and board[self.row-1][self.col-1] is None):
      self.row = random.randint(1, 3)
      self.col = random.randint(1, 3)
  ...
  def play(self):
    ...
    if(self.row and self.col and board[self.row-1][self.col-1] is None):
      print(self.row, self.col)
      drawXO(self.row, self.col)
      check_win()
      self.row = None
      self.col = None
      return True
...
def reset_game():
	global board, winner, XO, draw, player
	time.sleep(3)
	XO = 'x'
	player = f_player
